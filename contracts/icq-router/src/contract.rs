...
use drop_staking_base::state::icq_router::{
    Config, ConfigOptional, BALANCES, CONFIG, DELEGATIONS, NON_NATIVE_REWARD_BALANCES,
};
use drop_staking_base::{
    error::icq_router::{ContractError, ContractResult},
    msg::icq_router::{ExecuteMsg, InstantiateMsg, MigrateMsg, QueryMsg},
};
...
pub fn instantiate(
    ...
    let attrs = vec![
        ...
        attr("adapter", adapter.to_string()),
        attr("owner", owner),
    ];
    CONFIG.save(deps.storage, &Config { adapter })?;
