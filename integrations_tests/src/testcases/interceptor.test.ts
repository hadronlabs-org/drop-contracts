...
import { LidoInterchainInterceptor } from '../generated/contractLib';
import {
  QueryClient,
  StakingExtension,
...
import {
  ...
  Transaction,
  Transfer,
} from '../generated/contractLib/lidoInterchainInterceptor';

const InterceptorClass = LidoInterchainInterceptor.Client;

describe('Interchain interceptor', () => {
  const context: {
    park?: Cosmopark;
    contractAddress?: string;
    wallet?: DirectSecp256k1HdWallet;
    gaiaWallet?: DirectSecp256k1HdWallet;
    contractClient?: InstanceType<typeof InterceptorClass>;
    account?: AccountData;
    icaAddress?: string;
  ...
  beforeAll(async () => {
    context.park = await setupPark('interceptor', ['neutron', 'gaia'], true);
  ...
  it('instantiate', async () => {
    ...
    const instantiateRes = await LidoInterchainInterceptor.Client.instantiate(
      client,
      account.address,
      res.codeId,
      {
    ...
    context.contractClient = new LidoInterchainInterceptor.Client(
      client,
