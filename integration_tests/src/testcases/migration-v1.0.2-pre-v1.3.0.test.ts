...
describe('Migration from v1.0.2-pre to v1.3.0', () => {
  const context: {
    park?: Cosmopark;
  ...
  describe('state machine', () => {
    ...
    describe('first cycle', () => {
      ...
      it('bond lsm shares', async () => {
        ...
        expect(lsmSharesToRedeem).toEqual([]);
        expect(pendingLsmShares).toEqual([
          [
            tokenizedDenomOnNeutron,
            [`${context.validatorAddress}/1`, '100000', '100000'],
          ],
        ]);
      });
  ...
  it('validate unbonding batch', async () => {
    ...
    expect(batch).toEqual<DeprecatedUnbondBatch>({
      ...
      status: 'unbonding',
      total_dasset_amount_to_withdraw: '200000',
      expected_native_asset_amount: '200000',
      total_unbond_items: 1,
      unbonded_amount: null,
      withdrawn_amount: null,
    });
  ...
  describe('clearing LSM shares to proceed with migration', () => {
    it('tick', async () => {
      const {
        gaiaClient,
        neutronUserAddress,
    ...
    it('check lsm shares state', async () => {
      const {
        oldClients: { coreContractClient },
        tokenizedDenomOnNeutron,
      } = context;
      const lsmSharesToRedeem =
        await coreContractClient.queryLSMSharesToRedeem();
      const pendingLsmShares = await coreContractClient.queryPendingLSMShares();
    ...
    it('check lsm shares and delegations state', async () => {
      const {
        oldClients: { coreContractClient },
      } = context;
      const lsmSharesToRedeem =
          ...
          msgs: [
            {
              wasm: {
                execute: {
                  contract_addr:
                    context.oldClients.coreContractClient.contractAddress,
                  msg: Buffer.from(
                    JSON.stringify({
                      update_config: {
                        new_config: {
                          idle_min_interval: 31536000, // 1 year
                        },
                      },
                    }),
                  ).toString('base64'),
    ...
    it('tick', async () => {
      ...
      await expect(
        coreContractClient.tick(neutronUserAddress, 1.5, undefined, [
          {
            amount: '1000000',
            denom: 'untrn',
          },
  ...
  describe('running core tests after migration', () => {
                  ...
                  msg: Buffer.from(
                    JSON.stringify({
                      update_config: {
                        new_config: {
                          val_ref_contract:
                            context.valRefClient.contractAddress,
                        },
                      },
                    }),
                  ).toString('base64'),
    ...
    it('bond w/o receiver', async () => {
      ...
      expect(
        ...
        amount: String(Math.floor(1300_000 / context.exchangeRate)),
      });
    ...
    describe('state machine', () => {
      ...
      describe('first cycle', () => {
        it('tick', async () => {
          const { neutronUserAddress, neutronIBCDenom, puppeteerIcaAddress } =
            context;
        ...
        it('query all unbonding batches at once', async () => {
          ...
          expect(unbondBatches.length).toEqual(3);
          expect(nextPageKey).toBeNull();

          const [firstBatch, secondBatch, thirdBatch] = unbondBatches;
          ...
          expect(thirdBatch).toEqual<UnbondBatch>({
            slashing_effect: null,
            status: 'new',
            status_timestamps: expect.any(Object),
            expected_release_time: 0,
        ...
        it('query all unbonding batches with limit and page key', async () => {
          ...
          expect(secondNextPageKey).toEqual('2');
      ...
      describe('forth cycle', () => {
        it('validate NFT', async () => {
          ...
          expect(vouchers.tokens.length).toBe(3);
          ...
          expect(vouchers.tokens[1]).toBe(`1_${neutronUserAddress}_1`);
        ...
        it('try to withdraw before withdrawn', async () => {
          const { withdrawalVoucherContractClient, neutronUserAddress } =
            context;
          const tokenId = `1_${neutronUserAddress}_1`;
          await expect(
        ...
        it('fund withdrawal manager', async () => {
          ...
          const balance =
            await context.neutronClient.CosmosBankV1Beta1.query.queryAllBalances(
              pumpContractClient.contractAddress,
            );
          console.log('pumpContractClient');
          console.log(balance);

          await pumpContractClient.push(
            neutronUserAddress,
            {
              coins: [{ amount: '200000', denom: 'stake' }],
            },
            1.5,
            undefined,
            [{ amount: '20000', denom: 'untrn' }],
          ...
          await waitFor(async () => {
            ...
            console.log('withdrawalManagerContractClient');
      ...
      describe('fifth cycle (unbond before delegation)', () => {
        describe('prepare', () => {
          ...
          it('tick to clear LSM shares queue', async () => {
            const {
              gaiaClient,
              coreContractClient,
              neutronUserAddress,
