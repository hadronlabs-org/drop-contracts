...
import { DropProviderProposalsPoc } from 'drop-ts-client';
import {
  QueryClient,
  StakingExtension,
  BankExtension,
...
describe('POC Provider Proposals', () => {
  ...
  it('delegate tokens on gaia side and create text proposal', async () => {
    ...
    res = await context.park.executeInNetwork(
      'gaia',
      `gaiad tx gov submit-proposal --title test --description test --from ${context.gaiaUserAddress} --deposit 1000000stake --yes --chain-id testgaia --home=/opt --keyring-backend=test --output json`,
    );
    expect(res.exitCode).toBe(0);
