...
export interface DropProposalVotesSchema {
...
export class Client {
  ...
  queryMetrics = async (): Promise<Metrics> =>
    this.client.queryContractSmart(this.contractAddress, { metrics: {} });
  updateConfig = async (
    sender: string,
  ...
  updateActiveProposals = async (
    sender: string,
    args: UpdateActiveProposalsArgs,
    fee?: number | StdFee | 'auto',
    memo?: string,
    funds?: Coin[],
  ): Promise<ExecuteResult> => {
    ...
    return this.client.execute(
      sender,
      this.contractAddress,
      { update_active_proposals: args },
      fee || 'auto',
      memo,
      funds,
    );
  };
  updateVotersList = async (
    sender: string,
    args: UpdateVotersListArgs,
    fee?: number | StdFee | 'auto',
    memo?: string,
    funds?: Coin[],
  ): Promise<ExecuteResult> => {
